    ...
    public class FerngillCustomWeathers: Mod
    {
        ...
        private void OnLocationChange(object sender, WarpedEventArgs e)
        {
            if (e.NewLocation is IslandLocation)
            {
                ...
                if (Game1.dayOfMonth <= 14 && Randoms.NextDouble() < (ModOptions.FogChanceInEarlySummer / 2))
                    return;
                else if (Game1.dayOfMonth > 14 && Randoms.NextDouble() < (ModOptions.FogChanceInLateSummer / 2))
                    return;
            ...
            if (!(e.NewLocation is IslandLocation) && OurFog.HideInCurrentLocation)
            {
        ...
        private void MessageReceived(object sender, ModMessageReceivedEventArgs e)
        {
            ...
            if (e.Type == "FogSync")
            {
                ...
                OurFog.CreateWeather(message.type, true);
        ...
        private void TimeChanged(object sender, TimeChangedEventArgs e)
        {
            ...
            if ((FogToday && Game1.timeOfDay <= Utility.ModifyTime(Game1.getStartingToGetDarkTime(),-100) && Randoms.NextDouble() <= ModOptions.EveningWeatherFogChance) || ((Randoms.NextDouble() <= ModOptions.EveningWeatherFogChance / 4) && Game1.timeOfDay <= Utility.ModifyTime(Game1.getStartingToGetDarkTime(),-100)) && Game1.timeOfDay > 1500)
            {
                Monitor.Log("Firing off evening fog chance", LogLevel.Info);

                //evening fog
                var numTime = Randoms.Next(60, 190);
                ...
                OurFog.CreateWeather(todayType, true);
                OurFog.SetWeatherTime(startTime, endTime);
                SendFogMessage(todayType, startTime, endTime);
                FogToday = false;
            }
        }
        ...
        private void OnDayStart(object sender, DayStartedEventArgs e)
        {
            ...
            if (!Game1.isFestival() && !Game1.weddingToday)
            {
                CheckAndInitFog();
        ...
        private void CheckAndInitFog()
        {
            ...
            var timeChance = Randoms.NextDouble();
        ...
        {
            var fMessage = new FogMessage {StartTime = sTime, EndTime = eTime, type = f};
